function CGame(oData){
    ...
    this.onPlayerSelection = function(szSelected){
        ...
        _iCurCash += _iCurBet;
        var iCheckWin = Math.random()*100;
        
        $(s_oMain).trigger("bet_placed",_iCurBet);
        
        if(_iCurCash<_iCurBet*2){
            //USER MUST LOSE BECAUSE CASH IS LOWER THAN POSSIBLE WIN

            iCheckWin = WIN_OCCURRENCE+1;
        }
       
        if(iCheckWin < WIN_OCCURRENCE){
            //Win Case
    ...
    this.checkWin = function(oCard){
        if(!_bHighSelected && oCard.getRank() < 8){
            ...
            var oText = new CWinText(TEXT_WIN + " " + TEXT_CURRENCY + _iAmount + "!" , _oStartPoint.x, _iAmount);
            ...
            _iAmount = _iCurBet;
            var oText = new CWinText(TEXT_LOSE + " " + TEXT_CURRENCY + _iAmount + "!" , _oStartPoint.x, 0);
            _bGuess = false;
            if (oCard.getRank() < 8){
                _iNumLow++;
            } else {
